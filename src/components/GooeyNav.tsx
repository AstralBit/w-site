  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    setTimeout(() => {
      ...
      particle.style.cssText = `
        ...
        animation: globalParticle ${t}ms ease 1 forwards;
      ...
      point.style.cssText = `
        ...
        height: 20px;
        border-radius: 9999px;
        background: var(--color-${p.color}, white);
        transform-origin: center;
        animation: globalPoint ${t}ms ease 1 forwards;
        --scale: ${p.scale};
      `;
...
if (typeof document !== 'undefined') {
  const styleId = 'gooey-nav-global-particles';
...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const getActiveIndexFromPath = useCallback(() => {
    // 精确匹配当前路径
    const exactMatch = items.findIndex(item => item.href === pathname);
    if (exactMatch !== -1) return exactMatch;
    
    // 如果没有精确匹配，尝试前缀匹配（例如 /blog/xxx 匹配 /blog）
    const prefixMatch = items.findIndex(item => 
    ...
    return homeIndex !== -1 ? homeIndex : initialActiveIndex;
  }, [items, pathname, initialActiveIndex]);
  
  // 直接使用计算值，不需要 state
  const activeIndex = getActiveIndexFromPath();
            ...
            {items.map((item, index) => (
              <li
                ...
                onClick={(e) => {
                  ...
                  // 创建全局粒子容器（挂载到 body，不会随组件卸载）
                  createGlobalParticles(centerX, centerY, animationTime, particleCount, particleDistances, particleR, timeVariance, colors);
              ...
              >
                <span className="outline-none py-[0.6em] px-[1em] inline-block">
                  {item.label}
                </span>
              </li>
            ))}
